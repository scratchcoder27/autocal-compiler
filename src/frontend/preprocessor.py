...
class Preprocessor:
    def __init__(self):
        self.defines: dict[
            tuple[str, TokenType], tuple[list[str] | None, list[Token]]
        ] = {}
        ...
        while True:
            ...
            if current.type != TokenType.PREPROCESS:
                ...
                if macro_key in self.defines:
                    ...
                    if args is None:
                        self.input_code = (
                            self.input_code[: self.position]
                            + body
                            + self.input_code[self.position + 1 :]
                        )
                        continue
                    else:
                        if self.next().type == TokenType.LEFT_PAREN:
                            ...
                            while True:
                                ...
                                if tok.type == TokenType.EOF:
                                    raise PreprocessError(
                                        f"Unterminated argument list for macro '{macro_key[0]}'"
                                    )
                            ...
                            if (
                                len(args) == 0
                                and len(actual_args) == 1
                                and len(actual_args[0]) == 0
                            ):
                                actual_args = []

                            if len(actual_args) != len(args):
                                raise PreprocessError(
                                    f"Macro '{macro_key[0]}' expected {len(args)} arguments, got {len(actual_args)}"
                                )
                ...
                match directive:
                    case "ifdef":
                        ...
                        self.condition_stack.append(
                            name_key in self.defines if self.is_active else False
                        )
                    case "ifndef":
                        name_tok = self.advance()
                        name_key = (name_tok.lexeme, name_tok.type)
                        self.condition_stack.append(
                            name_key not in self.defines if self.is_active else False
                        )
                    case "ifequal":
                        val1 = self.advance()
                        val2 = self.advance()
                        self.condition_stack.append(
                            val1.lexeme == val2.lexeme if self.is_active else False
                        )
                    case "endif":
            ...
            match directive:
                        ...
                        if self.peek().type != TokenType.RIGHT_PAREN:
                            while True:
                                ...
                                if arg_token.type != TokenType.IDENTIFIER:
                                    raise PreprocessError(
                                        "Macro arguments must be valid identifiers"
                                    )
                    ...
                    while True:
                        ...
                        if val.type is TokenType.BITWISE_OR:
                            ...
                            match replace_type.lexeme:
                                ...
                                case _:
                                    raise PreprocessError(
                                        f"Invalid replacement in macro/define (|{replace_type.lexeme}|)"
                                    )
                            self.advance()
                            if self.peek().type is TokenType.BITWISE_OR:
                                ...
                                raise PreprocessError(
                                    f"No closing '|' in macro/define (|{replace_type.lexeme}|)"
                                )
                            continue
                ...
                case "error":
                    ...
                    raise PreprocessError(
                        " ".join(msg_tokens).strip() or "Error directive encountered"
                    )
                ...
                case "pragma":
                    if self.peek().type is TokenType.IDENTIFIER:
                        ...
                        raise PreprocessError(
                            f"Invalid pragma instruction, found {self.peek()}"
                        )
                ...
                case _:
                    raise PreprocessError(
                        f"Invalid preprocessor instruction: '{directive}'"
                    )
